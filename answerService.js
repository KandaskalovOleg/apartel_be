...
answerService.post('/api/submitNewAnswersEndpoint', async (req, res) => {
  try {
    const { password, answers, selectedPosition } = req.body;
    ...
    // Перевіряємо чи вже є дані по всім тестам
    if (!user.alltests) {
      // Якщо немає, створюємо об'єкт для всіх тестів
      user.alltests = {};
    }

    // Перевіряємо чи вже є дані по вибраній позиції
    if (!user.alltests[selectedPosition]) {
      // Якщо немає, створюємо об'єкт для вибраної позиції
      user.alltests[selectedPosition] = {
        marks: [], // Масив оцінок
        results: [] // Масив результатів
      };
    }

    // Отримуємо поточні відповіді та додаємо їх до масиву результатів
    const currentAnswers = {};
    for (const [questionIndex, options] of Object.entries(answers)) {
      const answer = {};
      for (const [optionIndex, value] of Object.entries(options)) {
    ...
    user.alltests[selectedPosition].results.push(currentAnswers);
    ...
    // Знаходимо позицію користувача
    const userPosition = positions.find(pos => pos.name === selectedPosition);
    if (!userPosition) {
      return res.status(404).json({ message: "Позицію користувача не знайдено." });
    }
    ...
    const totalQuestions = userPosition.pool.length;
    const mark = `${correctCount}/${totalQuestions}`;
    user.alltests[selectedPosition].marks.push(mark);
    
    // Зберігаємо оновлені дані користувача у файлі
    await fs.writeFile(usersDataFilePath, JSON.stringify(users, null, 2));

    res.json({ message: "Відповіді успішно збережено.", marks: mark });
  } catch (error) {
    console.error('Помилка під час обробки відповідей:', error);
    res.status(500).json({ message: "Помилка під час обробки відповідей." });
  }
